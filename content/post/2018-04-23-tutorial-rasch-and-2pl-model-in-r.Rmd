...
tags:
  ...
  - package
description: ''
bibliography: C:/Users/plieninger/Documents/Literature/lib.bib
csl: C:/Users/plieninger/Documents/Literature/apa.csl
output:
  blogdown::html_page:
    toc: true
---
...
```{r rasch-plots, cache = TRUE}
plotICC(res_rm_1, item.subset = 6)
...
From the plots of the ICCs, it seems like the "want"-items are easier than the "do"-items. We may further investigate this by comparing the means of the item difficulties. This shows that this is indeed the case. (The two means sum to 0, because of the identification constraint, see below.)
...
The estimated difficulty parameters (Dffclt) seem roughly comparable to those from the Rasch model. The discrimination parameters, which were all equal in the Rasch model, show some but not massive heterogeneity. This is also evident from the plot of the ICCs.
...
```{r 2pl, cache = TRUE}
# lmt() takes a formula (with a ~) as its first argument. The left-hand side
# must be the data and the right-hand side are the lantent variables (only one
# here).
...
Maximum a Posteriori (MAP) or Empirical Bayes (EB) estimates are available in the **ltm** package, as well as the closely related Expected a Posteriori (EAP) estimates. Both assume a normal distribution prior for the person parameters.

The high correlation among the estimates indicates that the methods lead to the same conclusions for the present data set, and this will often be the case.
